...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            if isinstance(x, list) and getattr(m, "need_unpack", False):
                x = m(*x)  # run modules that consume multiple feature maps directly
            else:
                x = m(x)  # run
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            C3x,
            RepC3,
            SimpleStem, VisionClueMerge, VSSBlock, HPCMBlock, XSSBlock
        }:
            c1, c2 = ch[f], args[0]

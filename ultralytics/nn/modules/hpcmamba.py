from .common_utils_hpcmamba import *
from .conv import Conv

__all__ = ("VSSBlock", "HPCMBlock", "SimpleStem", "VisionClueMerge", "XSSBlock")
...
class HPCMBlock(nn.Module):
    def __init__(
        ...
        self.mix = Conv(hidden_dim, hidden_dim, 1, 1)
        self.ds_conv1 = DepthwiseSeparableConv(hidden_dim // 2, hidden_dim // 2)
        self.ds_conv2 = DepthwiseSeparableConv(hidden_dim // 2, hidden_dim // 2)
        self.ds_conv3 = DepthwiseSeparableConv(hidden_dim // 2, hidden_dim // 2)
    ...
    def forward(self, input):
        ...
        x21 = self.ds_conv1(x2) + x2
        x22 = self.ds_conv2(x21) + x21
        x23 = self.ds_conv3(x22) + x22
        x_mix = self.mix(torch.cat([x1, x23], 1))
        if self.mlp_branch:
            return input + self.drop_path(self.mlp(x_mix) * input)
        return x_mix
